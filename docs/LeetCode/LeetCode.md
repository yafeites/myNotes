...
##### [LeetCode128最长连续序列](https://leetcode-cn.com/problems/longest-consecutive-sequence/)

Set存储数组元素,对于索引i如果num[i-1]不在set中进行计算

##### [LeetCode130被围绕的区域](https://leetcode-cn.com/problems/surrounded-regions/)

从最外层对O开始做广度优先遍历即可

##### [LeetCode131分割字符串](https://leetcode-cn.com/problems/palindrome-partitioning/)

动态规划+回溯

##### [LeetCode134加油站](https://leetcode-cn.com/problems/gas-station/)

维持一个sum,维持一个cur 一个index遍历数组,cur+=gas[i]-cost[i],cur小于0时index等于下一个数组位置 一圈以后 return sum>0?index:-1

##### [LeetCode136只出现一次的数字](https://leetcode-cn.com/problems/single-number/)

遍历数组异或,结果就是只出现一次的数字

##### [LeetCode138复制带随机指针的链表](https://leetcode-cn.com/problems/copy-list-with-random-pointer/)

用map维持原结点和复制结点的关系

##### [Leetcode139 单词拆分](https://leetcode-cn.com/problems/copy-list-with-random-pointer/)

动态规划,每一个help[i]表示从0到i是否为可拆分,并且这种拆分为无状态的

##### [LeetCode140单词拆分II](https://leetcode-cn.com/problems/word-break-ii/)

dp 找到每一个以i结束的位置可以拆分的最后一个单词,再用dfs进行组装即可
